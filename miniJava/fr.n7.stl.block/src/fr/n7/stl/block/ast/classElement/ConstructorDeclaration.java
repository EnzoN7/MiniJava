...
public class ConstructorDeclaration implements ClassElement {
    ...
    protected SymbolTable parametersSymbolTable;

    protected int offset;

    public ConstructorDeclaration(ClassAccessRight _accessRight, String _identifiant, List<ParameterDeclaration> _parameters, Block _corps) {
    ...
    public String toString() {
        String _result =  this.accessRight.toString() + this.identifiant + "(";
        if (parameters != null) {
    ...
    public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
        ...
        if (((HierarchicalScope<Declaration>) _scope).accepts(this)) {
            ...
            SymbolTable temp_parametersSymbolTable = new SymbolTable(_scope);
            boolean result = true;
            if (this.parameters != null) {
                for (ParameterDeclaration pd : this.parameters) {
                    temp_parametersSymbolTable.register(pd);
                }
            }
            this.parametersSymbolTable = temp_parametersSymbolTable;
            result = this.corps.collect(temp_parametersSymbolTable);
            return result;
        } else {
    ...
    public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
        return this.corps.resolve(this.parametersSymbolTable);
    }
