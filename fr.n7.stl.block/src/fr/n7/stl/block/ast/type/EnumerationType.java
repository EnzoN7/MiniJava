...
public class EnumerationType implements Type, Declaration {
	...
	public boolean equalsTo(Type _other) {
		if (_other instanceof EnumerationType) {
			return ((EnumerationType) _other).getName() == this.name
					&& this == ((EnumerationType) _other).getType();
		} else {
			return false;
		}
	}
	...
	public Type merge(Type _other) {
		if (this.compatibleWith(_other)) {
			return new EnumerationType(this.name,this.labels);
		} else {
	...
	public int length() {
		int _result = 0;
		for (LabelDeclaration lb : labels) {
			_result = lb.getType().length();
		}
		return _result;
	}
